<?php
/**
 * Load admin view for User Woocommerce Registration Form.
 *
...
use WCSMSOTP\Helper\MoException;
...
if ( ! class_exists( 'WooCommerceRegistrationForm' ) ) {
	...
	class WooCommerceRegistrationForm extends FormHandler implements IFormHandler {
		...
		protected function __construct() {
			...
			$this->form_name               = mowc_( 'WooCommerce Registration Form' );
			$this->is_form_enabled         = get_mo_wc_option( 'wc_default_enable' );
			$this->button_text             = get_mo_wc_option( 'wc_button_text' );
			$this->button_text             = ! MoUtility::is_blank( $this->button_text ) ? $this->button_text : mowc_( 'Click Here to send OTP' );
		...
		public function handle_form() {
			$this->is_ajax_form                = get_mo_wc_option( 'wc_is_ajax_form' );
			$this->otp_type                    = get_mo_wc_option( 'wc_enable_type' );
			$this->redirect_to_page            = get_mo_wc_option( 'wc_redirect' );
			$this->redirect_after_registration = get_mo_wc_option( 'wcreg_redirect_after_registration' );
			$this->restrict_duplicates         = get_mo_wc_option( 'wc_restrict_duplicates' );
		...
		public function miniorange_register_wc_script() {
			wp_register_script( 'mowcreg', MOV_WC_URL . 'includes/js/wcreg.min.js', array( 'jquery' ), MOV_WC_VERSION, true );
			wp_localize_script(
				'mowcreg',
				'mowcreg',
				array(
					...
					'buttontext' => mowc_( $this->button_text ),
		...
		public function custom_registration_redirect( $var ) {
			...
			if ( $this->redirect_after_registration && get_mo_wc_option( 'wc_default_enable' ) ) {
		...
		private function isPhoneVerificationEnabled() {
			...
			return VerificationType::BOTH === $otp_ver_type || VerificationType::PHONE === $otp_ver_type;
		}
		...
		private function assertPassword( $password ) {
			if ( get_mo_wc_option( 'woocommerce_registration_generate_password', '' ) === 'no' ) {
				if ( MoUtility::is_blank( $password ) ) {
					throw new MoException(
						'registration-error-invalid-password',
						mowc_( 'Please enter a valid account password.' ),
						204
					);
				}
			}
		...
		private function assertEmail( $email ) {
			if ( MoUtility::is_blank( $email ) || ! is_email( $email ) ) {
				throw new MoException(
					'registration-error-invalid-email',
					esc_html( mowc_( 'Please enter a valid email address.' ) ),
					202
				);
			}
			if ( email_exists( $email ) ) {
				throw new MoException(
					...
					esc_html( mowc_( 'An account is already registered with your email address. Please login.' ) ),
					203
				);
			}
		}
		...
		private function assertUserName( $username ) {
			if ( get_mo_wc_option( 'woocommerce_registration_generate_username', '' ) === 'no' ) {
				if ( MoUtility::is_blank( $username ) || ! validate_username( $username ) ) {
					throw new MoException(
						'registration-error-invalid-username',
						esc_html( mowc_( 'Please enter a valid account username.' ) ),
						200
					);
				}
				if ( username_exists( $username ) ) {
					throw new MoException(
						...
						esc_html( mowc_( 'An account is already registered with that username. Please choose another.' ) ),
						201
					);
				}
			}
		...
		private function processFormFields( $username, $email, $errors, $password, $phone, $data ) {
			global $wc_phone_logic;
			if ( strcasecmp( $this->otp_type, $this->type_phone_tag ) === 0 ) {
				if ( ! isset( $phone ) || ! MoUtility::validate_phone_number( $phone ) ) {
					return new WP_Error(
						'billing_phone_error',
						str_replace( '##phone##', $phone, $wc_phone_logic->mowc_get_otp_invalid_format_message() )
					);
				...
				if ( ! isset( $phone ) || ! MoUtility::validate_phone_number( $phone ) ) {
					return new WP_Error(
						'billing_phone_error',
						str_replace( '##phone##', sanitize_text_field( $data['billing_phone'] ), $wc_phone_logic->mowc_get_otp_invalid_format_message() )
					);
				} elseif ( $this->restrict_duplicates && $this->isPhoneNumberAlreadyInUse( $phone, 'billing_phone' ) ) {
					return new WP_Error( 'billing_phone_error', MoMessages::showMessage( MoMessages::PHONE_EXISTS ) );
				}
				$this->send_challenge( $username, $email, $errors, sanitize_text_field( $data['billing_phone'] ), VerificationType::BOTH, $password );
			}
			return $errors;
		}
		...
		public function mo_add_phone_field() {
			if ( ! did_action( 'woocommerce_register_form' ) || ! did_action( 'wcmp_vendor_register_form' ) ) {
                ...
                <label for="reg_billing_phone">
                    ' . esc_html( mowc_( 'Phone' ) ) . '
                    <span class="required">*</span>
                </label>
                <input type="text" class="input-text" 
                        name="billing_phone" id="reg_billing_phone" 
                        value="' . ( ! empty( $_POST['billing_phone'] ) ? esc_attr( sanitize_text_field( wp_unslash( $_POST['billing_phone'] ) ) ) : '' ) . '" /> </p>';// phpcs:ignore WordPress.Security.NonceVerification.Missing -- No need for nonce verification as the function is called on third party plugin hook
			}
		}
		...
		public function handle_form_options() {
			...
			update_mo_wc_option( 'wcreg_redirect_after_registration', $this->redirect_after_registration );
			update_mo_wc_option( 'wc_default_enable', $this->is_form_enabled );
			update_mo_wc_option( 'wc_enable_type', $this->otp_type );
			update_mo_wc_option( 'wc_restrict_duplicates', $this->restrict_duplicates );
			update_mo_wc_option( 'wc_redirect', $this->redirect_to_page );
			update_mo_wc_option( 'wc_is_ajax_form', $this->is_ajax_form );
			update_mo_wc_option( 'wc_button_text', $this->button_text );
		}
