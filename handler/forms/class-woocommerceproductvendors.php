<?php
/**
 * Load admin view for User Woocommerce Product Vendor Registration Form.
 *
...
if ( ! class_exists( 'WooCommerceProductVendors' ) ) {
	...
	class WooCommerceProductVendors extends FormHandler implements IFormHandler {
		...
		protected function __construct() {
			...
			$this->form_name               = mowc_( 'WooCommerce Product Vendor Registration Form' );
			$this->is_form_enabled         = get_mo_wc_option( 'wc_pv_default_enable' );
			$this->button_text             = get_mo_wc_option( 'wc_pv_button_text' );
		...
		public function handle_form() {
			$this->otp_type            = get_mo_wc_option( 'wc_pv_enable_type' );
			$this->restrict_duplicates = get_mo_wc_option( 'wc_pv_restrict_duplicates' );
		...
		public function sendAjaxOTPRequest() {
			...
			$mobile_number = MoUtility::sanitize_check( 'user_phone', $_POST );
			$user_email    = MoUtility::sanitize_check( 'user_email', $_POST );
		 ...
		 * @return WP_Error
		 */
		private function processFormFields( $username, $email, $errors, $password, $phone ) {
		...
		public function miniorange_register_wc_script() {
			wp_register_script( 'mowcpvreg', MOV_WC_URL . 'includes/js/wcpvreg.min.js', array( 'jquery' ), MOV_WC_VERSION, true );
			wp_localize_script(
				'mowcpvreg',
				'mowcpvreg',
				array(
					...
					'codeLabel'  => mowc_( 'Enter Verification Code' ),
				)
			);
		...
		public function handle_form_options() {
			...
			update_mo_wc_option( 'wc_pv_default_enable', $this->is_form_enabled );
			update_mo_wc_option( 'wc_pv_enable_type', $this->otp_type );
			update_mo_wc_option( 'wc_pv_restrict_duplicates', $this->restrict_duplicates );
			update_mo_wc_option( 'wc_pv_button_text', $this->button_text );
		}
	}
}
