<?php
/**
 * Load admin view for User WooCommerce CheckOut Form
 *
...
if ( ! class_exists( 'WooCommerceCheckOutForm' ) ) {
	...
	class WooCommerceCheckOutForm extends FormHandler implements IFormHandler {
		...
		protected function __construct() {
			...
			$this->form_name               = mowc_( 'WooCommerce Checkout Form' );
			$this->is_form_enabled         = get_mo_wc_option( 'wc_checkout_enable' );
			$this->button_text             = get_mo_wc_option( 'wc_checkout_button_link_text' );
			$this->button_text             = ! MoUtility::is_blank( $this->button_text ) ? $this->button_text
							: ( ! $this->popup_enabled ? mowc_( 'Verify Your Purchase' ) : mowc_( 'Place Order' ) );
		...
		public function handle_form() {
			...
			$this->disable_auto_login   = get_mo_wc_option( 'wc_checkout_disable_auto_login' );
			$this->payment_methods      = maybe_unserialize( get_mo_wc_option( 'wc_checkout_payment_type' ) );
			...
			$this->popup_enabled        = get_mo_wc_option( 'wc_checkout_popup' );
			$this->guest_check_out_only = get_mo_wc_option( 'wc_checkout_guest' );
			$this->show_button          = get_mo_wc_option( 'wc_checkout_button' );
			$this->otp_type             = get_mo_wc_option( 'wc_checkout_type' );
			$this->selective_payment    = get_mo_wc_option( 'wc_checkout_selective_payment' );
			$this->restrict_duplicates  = get_mo_wc_option( 'wc_checkout_restrict_duplicates' );
		...
		/**
		 * This function hooks to WooCommerce hook to edit posted Phone Number
		...
		private function routeData() {
			...
			if ( strcasecmp( trim( sanitize_text_field( wp_unslash( $_GET['option'] ) ) ), 'miniorange-wc-checkout' ) === 0 ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended -- Reading GET parameter for checking the option name, doesn't require nonce verification.
		...
		private function isPhoneNumberAlreadyInUse( $phone ) {
			...
			$key             = 'billing_phone';
			$current_user_id = strval( wp_get_current_user()->ID );
			...
			return MoUtility::is_blank( $results ) ? false : $results->user_id !== $current_user_id;
		}
		...
		public function add_custom_button( $order_id ) {
			...
			echo ',$mo("#miniorange_wc_otp_token_submit").click(function(o){
                    ...
                    if(requiredFields=="")
                    {
                        ...
                        $mo.ajax({
                            url:"' . esc_url( site_url() ) . '/?option=miniorange-wc-checkout",type:"POST",
		...
		public function my_custom_checkout_field( $checkout ) {
			...
			woocommerce_form_field(// phpcs:ignore intelephense.diagnostics.undefinedFunctions -- Default function of Woocommerce.
				...
				array(
					...
					'label'       => mowc_( 'Verify Code' ),
					'required'    => true,
					'placeholder' => mowc_( 'Enter Verification Code' ),
				),
			...
			echo ',$mo(".woocommerce-error").length>0&&$mo("html, body").animate({scrollTop:$mo("div.woocommerce").offset().top-50},1e3),$mo("#miniorange_wc_otp_token_submit").click(function(o){var e=$mo("input[name=billing_email]").val(),n=$mo("#billing_phone").val(),a=$mo("div.woocommerce");a.addClass("processing").block({message:null,overlayCSS:{background:"#fff",opacity:.6}}),$mo.ajax({url:"' . esc_url( site_url() ) . '/?option=miniorange-wc-checkout",type:"POST",data:{user_email:e, user_phone:n},crossDomain:!0,dataType:"json",success:function(o){ if(o.result=="success"){$mo(".blockUI").hide(),$mo("#mo_message").empty(),$mo("#mo_message").append(o.message),$mo("#mo_message").addClass("woocommerce-message").removeClass("woocommerce-error"),$mo("#mo_message").show()}else{$mo(".blockUI").hide(),$mo("#mo_message").empty(),$mo("#mo_message").append(o.message),$mo("#mo_message").addClass("woocommerce-error"),$mo("#mo_message").show();} ;},error:function(o,e,n){}}),o.preventDefault()});});</script></div>';
		}
		...
		private function showTextLinkOnPage() {
			if ( strcasecmp( $this->otp_type, $this->type_phone_tag ) === 0 ) {
				echo '<div style = "margin-bottom: 15px;" title="' . esc_html( mowc_( 'Please Enter a Phone Number to enable this link' ) ) . '">
                        <a  href="#" style="text-align:center;color:grey;pointer-events:initial;display:none;" 
                            id="miniorange_wc_otp_token_submit" 
                            class="" >' . esc_html( mowc_( $this->button_text ) ) . '
                        </a>
                   </div>';
			} else {
				echo '<div style = "margin-bottom: 15px;" title="' . esc_html( mowc_( 'Please Enter an Email Address to enable this link' ) ) . '">
                        <a  href="#" 
		 ...
		 * OTP Verification process.
		 */
		private function show_button_on_page() {
			if ( strcasecmp( $this->otp_type, $this->type_phone_tag ) === 0 ) {
				...
				. '" id="miniorange_wc_otp_token_submit" title="'
				. esc_attr( mowc_( 'Please Enter a Phone Number to enable this.' ) ) . '" value="';
			} else {
				echo '<input type="button" class="button alt" style="'
				. ( $this->popup_enabled ? 'float: right;line-height: 1;margin-right: 2em;padding: 1em 2em; display:none;' : 'display:none;width:100%;margin-bottom: 15px;' )
				. '" id="miniorange_wc_otp_token_submit" title="'
				. esc_attr( mowc_( 'Please Enter an Email Address to enable this.' ) ) . '" value="';
			}
			echo esc_attr( mowc_( $this->button_text ) ) . '"></input>';
		}
		 ...
		 * @return bool
		 */
		private function processPhoneNumber() {
		...
		public function enqueue_script_on_page() {
			$script_url = MOV_WC_URL . 'includes/js/wccheckout.min.js';
		...
		public function handle_form_options() {
			...
			if ( $this->basic_validation_check( BaseMessages::WC_CHECKOUT_CHOOSE ) ) {
				update_mo_wc_option( 'wc_checkout_restrict_duplicates', $this->restrict_duplicates );
				update_mo_wc_option( 'wc_checkout_disable_auto_login', $this->disable_auto_login );
				update_mo_wc_option( 'wc_checkout_enable', $this->is_form_enabled );
				update_mo_wc_option( 'wc_checkout_type', $this->otp_type );
				update_mo_wc_option( 'wc_checkout_guest', $this->guest_check_out_only );
				update_mo_wc_option( 'wc_checkout_button', $this->show_button );
				update_mo_wc_option( 'wc_checkout_popup', $this->popup_enabled );
				update_mo_wc_option( 'wc_checkout_selective_payment', $this->selective_payment );
				update_mo_wc_option( 'wc_checkout_button_link_text', $this->button_text );
				update_mo_wc_option( 'wc_checkout_payment_type', maybe_serialize( $payment_methods ) );
			}
		}
