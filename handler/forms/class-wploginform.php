<?php
/**
 * Load admin view for User WooCommerce Login Form.
 *
...
if ( ! class_exists( 'WPLoginForm' ) ) {
	...
	class WPLoginForm extends FormHandler implements IFormHandler {
		...
		protected function __construct() {
			...
			$this->form_name                 = mowc_( 'WooCommerce Login Form' );
			$this->is_form_enabled           = get_mo_wc_option( 'wp_login_enable' );
			$this->user_label                = get_mo_wc_option( 'wp_username_label_text' );
			$this->user_label                = $this->user_label ? mowc_( $this->user_label ) : mowc_( 'Username, E-mail or Phone No.' );
			$this->skip_password_check       = get_mo_wc_option( 'wp_login_skip_password' );
			$this->allow_login_through_phone = get_mo_wc_option( 'wp_login_allow_phone_login' );
			$this->skip_pass_fallback        = get_mo_wc_option( 'wp_login_skip_password_fallback' );
			$this->delay_otp                 = get_mo_wc_option( 'wp_login_delay_otp' );
			$this->delay_otp_interval        = get_mo_wc_option( 'wp_login_delay_otp_interval' );
		...
		public function handle_form() {
			$this->otp_type            = get_mo_wc_option( 'wp_login_enable_type' );
			$this->phone_key           = get_mo_wc_option( 'wp_login_key' );
			$this->save_phone_numbers  = get_mo_wc_option( 'wp_login_register_phone' );
			$this->by_pass_admin       = get_mo_wc_option( 'wp_login_bypass_admin' );
			$this->restrict_duplicates = get_mo_wc_option( 'wp_login_restrict_duplicates' );
		...
		public function isAdmin() {
			...
			$post_data = MoUtility::mowc_sanitize_array( $_POST );
		...
		public function miniorange_register_login_script() {
			wp_register_script( 'mologin', MOV_WC_URL . 'includes/js/loginform.min.js', array( 'jquery' ), MOV_WC_VERSION, true );
			wp_localize_script(
				'mologin',
				'movarlogin',
				array(
					...
					'buttontext'      => mowc_( 'Login with OTP' ),
		...
		public function mo_handle_mo_wp_login( $user, $username, $password ) {
			...
			$post_data = MoUtility::mowc_sanitize_array( $_POST ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- No need for nonce verification as the function is called on third party plugin hook
			$req_data  = MoUtility::mowc_sanitize_array( $_REQUEST ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- No need for nonce verification as the function is called on third party plugin hook.
			...
			if ( ! MoUtility::is_blank( $username ) ) {
				...
				$skip_otp_process = $this->skipOTPProcess( $password, $post_data, $user );
		...
		private function getUser( $username, $post_data, $password = null ) {
			...
			return $user ? $user : new WP_Error( 'INVALID_USERNAME', mowc_( ' <b>ERROR:</b> Invalid UserName. ' ) );
		}
		...
		private function askPhoneAndStartVerification( $user, $key, $username, $phone_number ) {
			...
			if ( ! $this->savePhoneNumbers() ) {
				miniorange_wc_site_otp_validation_form(
					null,
					null,
					null,
		...
		private function isLoginWithOTP( $post_data, $user_roles = array() ) {
			$login_with_otp_text = mowc_( 'Login with OTP' );
		 ...
		 * @return bool
		 */
		private function skipOTPProcess( $password, $post_data, $user ) {
			$user_meta = get_userdata( $user->data->ID );
		...
		public function handle_form_options() {
			...
			update_mo_wc_option( 'wp_login_enable_type', $this->otp_type );
			update_mo_wc_option( 'wp_login_enable', $this->is_form_enabled );
			update_mo_wc_option( 'wp_login_register_phone', $this->save_phone_numbers );
			update_mo_wc_option( 'wp_login_bypass_admin', $this->by_pass_admin );
			update_mo_wc_option( 'wp_login_key', $this->phone_key );
			update_mo_wc_option( 'wp_login_allow_phone_login', $this->allow_login_through_phone );
			update_mo_wc_option( 'wp_login_restrict_duplicates', $this->restrict_duplicates );
			update_mo_wc_option( 'wp_login_skip_password', $this->skip_password_check && $this->is_form_enabled );
			update_mo_wc_option( 'wp_login_skip_password_fallback', $this->skip_pass_fallback );
			update_mo_wc_option( 'wp_username_label_text', $this->user_label );
			update_mo_wc_option( 'wp_login_delay_otp', $this->delay_otp && $this->is_form_enabled );
			update_mo_wc_option( 'wp_login_delay_otp_interval', $this->delay_otp_interval );
		}
