...
if ( ! class_exists( 'WcProfileForm' ) ) {
	...
	class WcProfileForm extends FormHandler implements IFormHandler {
		...
		protected function __construct() {
			...
			$this->form_name               = mowc_( 'WooCommerce Account Details Form' );
			$this->is_form_enabled         = get_mo_wc_option( 'wc_profile_enable' );
			$this->restrict_duplicates     = get_mo_wc_option( 'wc_profile_restrict_duplicates' );
			$this->button_text             = get_mo_wc_option( 'wc_profile_button_text' );
			$this->button_text             = ! MoUtility::is_blank( $this->button_text ) ? $this->button_text : mowc_( 'Click Here to send OTP' );
			$this->phone_key               = get_mo_wc_option( 'wc_profile_phone_key' );
		...
		public function handle_form() {
			$this->otp_type = get_mo_wc_option( 'wc_profile_enable_type' );
		...
		public function miniorange_wc_ac_script() {
			wp_register_script( 'mowcac', MOV_WC_URL . 'includes/js/mowcac.min.js', array( 'jquery' ), MOV_WC_VERSION, true );
			wp_localize_script(
				'mowcac',
				'mowcac',
				array(
		...
		/**
		 * Handles saving all the Account Details Form related options by the admin.
		 */
		public function handle_form_options() {
			...
			update_mo_wc_option( 'wc_profile_enable', $this->is_form_enabled );
			update_mo_wc_option( 'wc_profile_enable_type', $this->otp_type );
			update_mo_wc_option( 'wc_profile_button_text', $this->button_text );
			update_mo_wc_option( 'wc_profile_restrict_duplicates', $this->restrict_duplicates );
			update_mo_wc_option( 'wc_profile_phone_key', $this->phone_key );
		}
	}
}