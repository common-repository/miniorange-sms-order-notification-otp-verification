...
if ( ! class_exists( 'WooCommerceBilling' ) ) {
	...
	class WooCommerceBilling extends FormHandler implements IFormHandler {
		...
		protected function __construct() {
			...
			$this->form_name               = mowc_( 'WooCommerce Billing Address Form' );
			$this->is_form_enabled         = get_mo_wc_option( 'wc_billing_enable' );
			$this->button_text             = get_mo_wc_option( 'wc_billing_button_text' );
		...
		public function handle_form() {
			$this->restrict_duplicates = get_mo_wc_option( 'wc_billing_restrict_duplicates' );
			$this->otp_type            = get_mo_wc_option( 'wc_billing_type_enabled' );
		...
		public function handle_form_options() {
			...
			if ( $this->basic_validation_check( BaseMessages::WC_BILLING_CHOOSE ) ) {
				update_mo_wc_option( 'wc_billing_enable', $this->is_form_enabled );
				update_mo_wc_option( 'wc_billing_type_enabled', $this->otp_type );
				update_mo_wc_option( 'wc_billing_restrict_duplicates', $this->restrict_duplicates );
			}
		}
	}
}
