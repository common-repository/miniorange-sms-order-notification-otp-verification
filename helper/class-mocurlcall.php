...
if ( ! class_exists( 'MocURLCall' ) ) {
	...
	class MocURLCall {
		...
		public static function mo_send_otp_token( $auth_type, $email = '', $phone = '' ) {
			...
			$customer_key = ! MoUtility::is_blank( get_mo_wc_option( 'admin_customer_key' ) )
							? get_mo_wc_option( 'admin_customer_key' ) : MoConstants::DEFAULT_CUSTOMER_KEY;
			$api_key      = ! MoUtility::is_blank( get_mo_wc_option( 'admin_api_key' ) )
							? get_mo_wc_option( 'admin_api_key' ) : MoConstants::DEFAULT_API_KEY;
			$fields       = array(
		...
		public static function submit_contact_us( $q_email, $q_phone, $query ) {
			...
			$mo_user         = get_mo_wc_option( 'admin_email' );
			$url             = MoConstants::HOSTNAME . '/moas/rest/customer/contact-us';
			$query           = '[' . MoConstants::QUERY_NAME . '(' . MoConstants::PLUGIN_TYPE . ') ](' . $mo_user . '): ' . $query;
			$customer_key    = ! MoUtility::is_blank( get_mo_wc_option( 'admin_customer_key' ) )
								? get_mo_wc_option( 'admin_customer_key' ) : MoConstants::DEFAULT_CUSTOMER_KEY;
			$api_key         = ! MoUtility::is_blank( get_mo_wc_option( 'admin_api_key' ) )
								? get_mo_wc_option( 'admin_api_key' ) : MoConstants::DEFAULT_API_KEY;
			$fields          = array(
				'firstName' => $mo_current_user->user_firstname,
				'lastName'  => $mo_current_user->user_lastname,
		 ...
		 * @param string $license_type License type of customer.
		 */
