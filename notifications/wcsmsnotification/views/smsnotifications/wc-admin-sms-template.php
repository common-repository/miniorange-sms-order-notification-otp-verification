...
echo '				<div>
						<div class="mt-mowc-3 flex flex-col gap-mowc-6">';
						wp_nonce_field( 'mo_admin_actions' );
						echo '

							<div class="w-full flex">
								<div class="flex-1">
									<h5 class="mowc-title flex items-center gap-mowc-2">
										' . esc_html( 'Recipients' ) . '
									</h5>
									<p class="mowc-caption mt-mowc-2">Admin Phone numbers on which Notifications will be received.</p>
								</div>
								<div class="flex-1 flex flex-wrap">
									<input type="text" name="wc_admin_order_status_notif_recipient" id="wc_admin_order_status_notif_recipient" value="' . esc_attr( $recipient_value ) . '" class="w-full mowc-input" placeholder="' . esc_html( mowc_( 'Enter semi-colon (;) to separate multiple phone numbers.' ) ) . '"/>
								';
						...
						echo '							</div>
							...
							<div class="w-full flex">
								<div class="flex-1">
									<h5 class="mowc-title">SMS Template</h5>
									<p class="mowc-caption mt-mowc-2">' . esc_html( $sms_settings->page_description ) . '</p>
								</div>
								<div class="flex-1">
 									<textarea id="' . esc_attr( $textarea_tag ) . '" class="mowc-textarea w-full mowc_remaining_characters"
							...
							<div class="w-full flex">
								<div class="flex-1">
									<h5 class="mowc-title flex items-center gap-mowc-2">' . wp_kses(
											$tag_icon,
											array(
												'svg'  => array(
													'width' => true,
									...
									<p class="mowc-caption mt-mowc-2"></p>
								</div>
								<div class="flex-1 mowc-tags-section">';
						foreach ( $tags as $val ) {
							echo '<div class="mowc-tag bg-slate-100">' . esc_attr( $val ) . ' </div>';
						}
						...
						echo '							</div>
							...
							<div class="w-full flex">
								<div class="flex-1">
									<h5 class="mowc-title flex items-center gap-mowc-2">' . wp_kses( $crown_icon, MoUtility::mowc_allow_svg_array() ) . ' Premium Tags</h5>
									<p class="mowc-caption mt-mowc-2"></p>
								</div>
								<div class="flex-1 mowc-tags-section">';
						foreach ( $prem_tags as $val ) {
							echo '<div class="mowc-tag bg-yellow-100">' . esc_attr( $val ) . ' </div>';
						}
